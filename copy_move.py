...
class Copy_move(nn.Module):
    def __init__(self,arch,resume,attribute):
        super(Copy_move, self).__init__()
    ...
    def forward(self, xt,xp):
        ...
        xt = torch.clamp(xt +noise1+128.,0,255)
        xp = torch.clamp(xp -noise1+128.,0,255)
...
def copy_move():
    ...
    return Copy_move(arch,resume,attribute())