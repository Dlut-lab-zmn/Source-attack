...
from advertorch.defenses import MedianSmoothing2D, AverageSmoothing2D,GaussianSmoothing2D,JPEGFilter
...
class cross_entropy_loss(object):
    ...
    def __call__(self, input, label):
        ...
        #loss = torch.sum(-label[:,:14]* torch.log(prob[:,:14] + 1e-8))
...
if args.GCE:
    adversary = LinfPGDAttack(
        ...
        adversary = L2BasicIterativeAttack(net, loss_fn=csl, eps=args.eps, nb_iter=10, eps_iter=10.0, clip_min=0.0, clip_max=255., targeted=False)
...
for iter in range(iters):
    ...
    if iter % args.print_freq == 0:
        ...
        #top3 = accuracy(outt3.data[:,:14], labelt[:,:14], 1)
