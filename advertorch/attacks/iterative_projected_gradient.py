...
def perturb_iterative(xvar, yvar, predict, nb_iter, eps, eps_iter, loss_fn,
    ...
    for ii in range(nb_iter):
        conB_Fea = predict(xvar + delta)[2]
        loss = loss_fn(conB_Fea, yvar)
        if minimize:
            loss = -loss
...
class PGDAttack(Attack, LabelMixin):
    ...
    def __init__(
            self, predict, loss_fn=None, eps=0.2, nb_iter=40,
...
class L2BasicIterativeAttack(PGDAttack):
    ...
    def __init__(self, predict, loss_fn=None, eps=0.2, nb_iter=10,
...
class MomentumIterativeAttack(Attack, LabelMixin):
    ...
    def perturb(self, x, y=None):
        ...
        for i in range(self.nb_iter):
            ...
            imgadv = x + delta
            conD_Fea,conM_Fea,conB_Fea = self.predict(imgadv)
            loss = self.loss_fn(conB_Fea, y)
            if self.targeted:
                loss = -loss
            loss.backward()
