...
def cross_entropy_loss(out1,out2,out3,label,mlabel,clabel,with_noise = False):
    ...
    if not with_noise:
        ...
        cost4 = torch.sum(torch.abs(prob3[:, :14] - prob[:, :14]))
        cost5 = torch.sum(torch.abs(prob3[:, :19] - prob2[:, :19]))
        loss = 0.2 * loss1 + 0.5 * loss2 + 0.3* loss3# + 0.1* cost4+0.1*cost5
    else:
    
        cost4 = torch.sum(torch.abs(prob3[:, :14] - prob[:, :14]))
        cost5 = torch.sum(torch.abs(prob3[:, :19] - prob2[:, :19]))
        loss = -0.01*(cost4 + cost5)#
...
def train(train_num, model, criterion, optimizer, epoch):
    ...
    for iter in range(iters):
        ...
        loss1 = criterion(out,out2,out3, label,mlabel,clabel)
        noise = torch.normal(0,5,input.size()).cuda()
        adv_inputs = torch.clamp(input+noise,0,255)
        outb,outm,outd = model(adv_inputs)
        loss2 = criterion(outb,outm,outd, label, mlabel, clabel,with_noise=True);print(loss2)
        loss = loss1 + loss2
