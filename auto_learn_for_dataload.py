...
import shutil
import time
from dataload import load_file_list, load_test_list, \
    get_batch, get_test
import torch
...
model_names = sorted(name for name in auto_learn_womean.__dict__
                     if name.islower() and not name.startswith("__")
                     and name.startswith("vgg")
                     and callable(auto_learn_womean.__dict__[name]))
print(model_names)
...
parser.add_argument('--save-dir', dest='save_dir',
                    ...
                    default='dataload_auto_learn_attack_womean', type=str)
...
def cross_entropy_loss(out1, out2, out3, label, mlabel, clabel):
    ...
    loss4 = torch.sum(-label[:,:19]*torch.log(prob4 + 1e-8))
...
def main():
    ...
    model = auto_learn_womean.__dict__[args.arch]()
...
def validate(test_num, model, criterion):
    ...
    for iter in range(iters//2):
        ...
        inputp, labelp, mlabelp, clabelp = get_test(args.batch_size)
        inputt = torch.FloatTensor(inputt)
        inputp = torch.FloatTensor(inputp)
        labelt = torch.FloatTensor(labelt)
        labelp = torch.FloatTensor(labelp)
    ...
    f.write('TE{top1.avg:.3f}'.format(top1=top1))
    f.write('TE{top2.avg:.3f}'.format(top2=top2))
    f.write('TE{top3.avg:.3f}'.format(top3=top3))
    f.write('TE{top4.avg:.3f}'.format(top4=top4))
    f.write('TE{top5.avg:.3f}'.format(top5=top5))
    f.write('TE{top6.avg:.3f}'.format(top6=top6))
    f.write('TE{top7.avg:.3f}'.format(top7=top7))
    f.write('TE{top8.avg:.3f}'.format(top8=top8))
    f.write('TE{top9.avg:.3f}'.format(top9=top9))
    f.write('TE{top10.avg:.3f}'.format(top10=top10))
    f.write('TE{losses.avg:.3f}\n'.format(losses=losses))
